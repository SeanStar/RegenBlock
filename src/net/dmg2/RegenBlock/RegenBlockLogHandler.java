...
public class RegenBlockLogHandler {
	...
	public void sendPlayerRegionCreate(Player player, String regionName, int respawnTime) {
		...
		sendPlayerNormal(player, "Respawn time: " + respawnTime + "s.");
	...
	public void listRegion(Player player, Set<String> listRegion) {
		for (String regionName : listRegion) {
			...
			String left = plugin.config.getRegionLeft(regionName);
			String right = plugin.config.getRegionRight(regionName);
			//Get world name
			String worldName = plugin.config.getRegionWorldName(regionName);
			//Get re-spawn time
			int respawnTime = plugin.config.getRegionRespawnTime(regionName);
			//Message the player
